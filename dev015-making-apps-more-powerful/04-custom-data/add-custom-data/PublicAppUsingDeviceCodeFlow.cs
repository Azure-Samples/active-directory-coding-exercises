...
{
    /// <summary>
    /// Base class for public client applications in the sample that leverage the DeviceCodeFlow to 
    /// authenticate a user
    /// </summary>
    public class PublicAppUsingDeviceCodeFlow
    {
        protected PublicAppUsingDeviceCodeFlow(PublicClientApplication app)
        {
            App = app;
        }
        ...
        async Task<AuthenticationResult> GetTokenForWebApiUsingDeviceCodeFlowAsync(IEnumerable<string> scopes)
        {
            AuthenticationResult result;
            try
            {
